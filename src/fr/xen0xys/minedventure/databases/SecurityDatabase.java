...
public class SecurityDatabase {
    ...
    private final String account_table_name = "minedventure_accounts_security";
    ...
    public void initializeTableIfNotInitialized(){
        if(!this.database_provider.isTableInitialised(this.account_table_name)){
            this.database_provider.initializeTable(this.account_table_name, "id INT PRIMARY KEY AUTO_INCREMENT, minedventure_account_id INT, password VARCHAR(100), ip VARCHAR(30), last_login BIGINT, has_session INT, FOREIGN KEY (minedventure_account_id) REFERENCES minedventure_accounts(id) ON DELETE CASCADE ON UPDATE NO ACTION");
        }
    }

    public boolean isPasswordEquals(String minecraft_name, String given_password){
        return false;
    }

    public boolean setPassword(String minecraft_name, String new_password){
        this.initializeTableIfNotInitialized();
        String encrypted_password = PluginUtils.encryptPassword(new_password);
        int id = new AccountsDatabase().getIdFromMinecraftName(minecraft_name);
        if(id != -1){
            try{
                String query = String.format("SELECT id FROM %s WHERE minedventure_account_id='%s'", this.account_table_name, id);
    ...
    public boolean checkPassword(String minecraft_name, String password){
        ...
        if(id != -1){
            try {
                String encrypted_password = PluginUtils.encryptPassword(password);
                String query = String.format("SELECT password FROM %s WHERE minedventure_account_id='%d'", this.account_table_name, id);
                ResultSet rs = this.database_provider.executeQuery(query);
                if(rs.next()){
    ...
    public boolean loginUser(String minecraft_name, String ip){
        int id = new AccountsDatabase().getIdFromMinecraftName(minecraft_name);
        if(id != -1){
            String query = String.format("UPDATE %s SET ip='%s', last_login='%s', has_session=1 WHERE minedventure_account_id='%d'", this.account_table_name, ip, PluginUtils.getCurrentTimestamp(), id);
            return this.database_provider.executeUpdateQuery(query);
        }
        return false;
    }
    ...
    public boolean logoutUser(String minecraft_name){
        int id = new AccountsDatabase().getIdFromMinecraftName(minecraft_name);
        if(id != -1){
            String query = String.format("UPDATE %s SET has_session=0 WHERE minedventure_account_id='%d'", this.account_table_name, id);
            return this.database_provider.executeUpdateQuery(query);
        }
        return false;
    }
    ...
    public boolean hasSession(String minecraft_name, String ip){
        ...
        if(id != -1){
            try {
                String query = String.format("SELECT last_login, ip, has_session FROM %s WHERE minedventure_account_id='%d'", this.account_table_name, id);
                ResultSet rs = this.database_provider.executeQuery(query);
                if(rs.next()){
